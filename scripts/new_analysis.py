...
def load_cache(
    ...
    multiscript_run: str = "multiscript_bigrams",
    wild_run: str = "wild_bigrams",
):
    ...
    wild_dir = root / "cache" / "improbable_bigrams" / model_name / wild_run
    ...
    cache = {
        ...
        "multiscript_manifest": load_json(multiscript_dir / "manifest.json"),
        "wild_manifest": load_json(wild_dir / "manifest.json"),
        "improbable_index": [
            json.loads(line)
        ...
        "multiscript_index": [
            json.loads(line)
            for line in (multiscript_dir / "index.jsonl").read_text(encoding="utf-8").splitlines()
            if line.strip()
        ],
        "wild_index": [
            json.loads(line)
            for line in (wild_dir / "index.jsonl").read_text(encoding="utf-8").splitlines()
            if line.strip()
        ],
        ...
        "multiscript_scores_all": torch.load(
            multiscript_dir / "scores" / "per_example_all.pt", map_location="cpu"
        ),
        "multiscript_dla_all": torch.load(
            multiscript_dir / "dla" / "per_example_all_p1.pt", map_location="cpu"
        ),
        "wild_scores_all": torch.load(
            wild_dir / "scores" / "per_example_all.pt", map_location="cpu"
        ),
        "wild_dla_all": torch.load(
            wild_dir / "dla" / "per_example_all_p1.pt", map_location="cpu"
        ),
    }
    ...
    cache["wild_count"] = len(cache["wild_scores_all"]["examples"])
    return cache
...
def dataset_overview(cache) -> pd.DataFrame:
    ...
    multiscript_hall = sum(
        1 for entry in cache["multiscript_index"] if entry["second_token_hallucination"]
    )
    multiscript_copy = sum(
        1 for entry in cache["multiscript_index"] if entry["copy_success"]
    )
    wild_hall = sum(
        1 for entry in cache["wild_index"] if entry["second_token_hallucination"]
    )
    wild_copy = sum(1 for entry in cache["wild_index"] if entry["copy_success"])

    improbable_examples = len(cache["improbable_task_ids"])
    concept_examples = cache["concept_count"]
    random_examples = cache["random_count"]
    multiscript_examples = cache["multiscript_count"]
    wild_examples = cache["wild_count"]
    ...
    return pd.DataFrame(
        [
            ...
            {
                "dataset": "Multiscript bigram cache",
                "examples": multiscript_examples,
                "hallucinated_second_token": multiscript_hall,
                "copied": multiscript_copy,
            },
            {
                "dataset": "Wild bigram cache",
                "examples": wild_examples,
                "hallucinated_second_token": wild_hall,
                "copied": wild_copy,
            },
        ]
    )
...
def _condition_series(
    ...
    multiscript_tensor: torch.Tensor,
    wild_tensor: torch.Tensor,
    mask: torch.Tensor,
    cache,
) -> dict[str, np.ndarray]:
    return {
        ...
        "Multiscript bigrams": aggregate_metric(multiscript_tensor, mask),
        "Wild bigrams": aggregate_metric(wild_tensor, mask),
    }
...
def token_ntm_series(cache, k: int, weighted: bool = False) -> dict[str, np.ndarray]:
    ...
    multiscript = cache["multiscript_scores_all"][field]
    wild = cache["wild_scores_all"][field]
    return _condition_series(
        improbable, concepts, random_scores, multiscript, wild, token_heads, cache
    )
...
def concept_ltm_series(cache, k: int, weighted: bool = False) -> dict[str, np.ndarray]:
    ...
    wild = cache["wild_scores_all"][field]
    return _condition_series(
        improbable, concepts, random_scores, multiscript, wild, concept_heads, cache
    )
...
def token_correct_dla_series(cache, k: int) -> dict[str, np.ndarray]:
    ...
    multiscript = cache["multiscript_dla_all"]["correct_token_dla"]
    wild = cache["wild_dla_all"]["correct_token_dla"]
    return _condition_series(
        improbable, concepts, random_scores, multiscript, wild, token_heads, cache
    )
...
def build_pairwise_summary(cache, k: int) -> pd.DataFrame:
    ...
    for metric, series in metric_getters:
        ...
        multiscript = series["Multiscript bigrams"]
        wild = series["Wild bigrams"]
        for comparison, right in (
            ...
            ("Hallucinated - Multiscript", multiscript),
            ("Hallucinated - Wild", wild),
        ):
...
def interpretation_markdown(cache, k: int = 32) -> str:
    ...
    ntm_hms = row("Token NTM (raw)", "Hallucinated - Multiscript")
    ntm_hw = row("Token NTM (raw)", "Hallucinated - Wild")
    ...
    dla_hms = row("Token correct-token DLA", "Hallucinated - Multiscript")
    dla_hw = row("Token correct-token DLA", "Hallucinated - Wild")
    ...
    return "\n".join(
        [
            ...
            (
                f"- Hallucinated vs multiscript NTM: `{ntm_hms['diff']:.4f}` "
                f"(95% CI `{ntm_hms['ci_low']:.4f}` to `{ntm_hms['ci_high']:.4f}`)"
            ),
            (
                f"- Hallucinated vs multiscript correct-token DLA: `{dla_hms['diff']:.4f}` "
                f"(95% CI `{dla_hms['ci_low']:.4f}` to `{dla_hms['ci_high']:.4f}`)"
            ),
            (
                f"- Hallucinated vs wild NTM: `{ntm_hw['diff']:.4f}` "
                f"(95% CI `{ntm_hw['ci_low']:.4f}` to `{ntm_hw['ci_high']:.4f}`)"
            ),
            (
                f"- Hallucinated vs wild correct-token DLA: `{dla_hw['diff']:.4f}` "
                f"(95% CI `{dla_hw['ci_low']:.4f}` to `{dla_hw['ci_high']:.4f}`)"
            ),
            (
...
def concept_interpretation_markdown(cache, k: int = 32) -> str:
    ...
    ltm_hms = row("Concept LTM (raw)", "Hallucinated - Multiscript")
    ltm_hw = row("Concept LTM (raw)", "Hallucinated - Wild")
    ...
    dla_hms = row("Concept correct-token DLA", "Hallucinated - Multiscript")
    dla_hw = row("Concept correct-token DLA", "Hallucinated - Wild")
    ...
    return "\n".join(
        [
            ...
            (
                f"- Hallucinated vs multiscript LTM: `{ltm_hms['diff']:.4f}` "
                f"(95% CI `{ltm_hms['ci_low']:.4f}` to `{ltm_hms['ci_high']:.4f}`)"
            ),
            (
                f"- Hallucinated vs multiscript correct-token DLA: `{dla_hms['diff']:.4f}` "
                f"(95% CI `{dla_hms['ci_low']:.4f}` to `{dla_hms['ci_high']:.4f}`)"
            ...
            (
                f"- Hallucinated vs wild LTM: `{ltm_hw['diff']:.4f}` "
                f"(95% CI `{ltm_hw['ci_low']:.4f}` to `{ltm_hw['ci_high']:.4f}`)"
            ),
            (
                f"- Hallucinated vs wild correct-token DLA: `{dla_hw['diff']:.4f}` "
                f"(95% CI `{dla_hw['ci_low']:.4f}` to `{dla_hw['ci_high']:.4f}`)"
...
def plot_distribution_panels(cache, k: int = 32, save_path: Path | None = None):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(14, 6.0), constrained_layout=False)
    ...
    fig.subplots_adjust(top=0.78, wspace=0.26, bottom=0.20)
    if save_path is not None:
        save_path.parent.mkdir(parents=True, exist_ok=True)
        fig.savefig(save_path, bbox_inches="tight")
    return fig, axes
...
def plot_top_head_heatmap(cache, top_k: int = 16, save_path: Path | None = None):
    ...
    multiscript_ntm = cache["multiscript_scores_all"]["ntm_raw"]
    wild_ntm = cache["wild_scores_all"]["ntm_raw"]
    ...
    multiscript_correct_dla = cache["multiscript_dla_all"]["correct_token_dla"]
    wild_correct_dla = cache["wild_dla_all"]["correct_token_dla"]
    ...
    for layer, head_idx in heads:
        ...
        ntm_matrix.append(
            [
                ...
                float(multiscript_ntm[:, layer, head_idx].mean().item()),
                float(wild_ntm[:, layer, head_idx].mean().item()),
            ]
        )
        dla_matrix.append(
            [
                ...
                float(multiscript_correct_dla[:, layer, head_idx].mean().item()),
                float(wild_correct_dla[:, layer, head_idx].mean().item()),
            ]
        )
    ...
    fig, axes = plt.subplots(1, 2, figsize=(15.5, max(5, 0.35 * top_k)), constrained_layout=True)
    ntm_image = axes[0].imshow(ntm_matrix, aspect="auto", cmap="viridis")
    axes[0].set_title(f"Top-{top_k} token heads: NTM")
    axes[0].set_xticks(range(6))
    axes[0].set_xticklabels(
        ["Hall", "Copied", "Concepts", "Random", "Multiscript", "Wild"]
    )
    ...
    plt.setp(axes[1].get_xticklabels(), rotation=22, ha="right")
...
def plot_concept_head_heatmap(cache, top_k: int = 16, save_path: Path | None = None):
    ...
    multiscript_ltm = cache["multiscript_scores_all"]["ltm_raw"]
    wild_ltm = cache["wild_scores_all"]["ltm_raw"]
    correct_dla = cache["improbable_dla_all"]["correct_token_dla"]
    wrong_dla = cache["improbable_dla_all"]["predicted_token_dla"]
    concept_correct_dla = cache["concept_dla_all"]["correct_token_dla"]
    random_correct_dla = cache["random_dla_all"]["correct_token_dla"]
    ...
    for layer, head_idx in heads:
        ...
        ltm_matrix.append(
            [
                ...
                float(multiscript_ltm[:, layer, head_idx].mean().item()),
                float(wild_ltm[:, layer, head_idx].mean().item()),
            ]
        )
        dla_matrix.append(
            [
...
def _dla_source_tensor(cache, source: str) -> tuple[torch.Tensor, str]:
    ...
    if source == "multiscript_correct":
        tensor = cache["multiscript_dla_all"]["correct_token_dla"]
        return tensor, "Multiscript-bigram correct-token DLA"
    if source == "wild_correct":
        tensor = cache["wild_dla_all"]["correct_token_dla"]
        return tensor, "Wild-bigram correct-token DLA"
