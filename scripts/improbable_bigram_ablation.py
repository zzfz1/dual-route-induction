...
Usage:
    ...
    # OLMo-3 / Qwen3 (local):
    uv run python dual-route-induction/scripts/improbable_bigram_ablation.py --model allenai/Olmo-3-1025-7B
    uv run python dual-route-induction/scripts/improbable_bigram_ablation.py --model Qwen/Qwen3-8B
"""
...
def load_mean_activations(model_name: str) -> torch.Tensor:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run compute_mean_activations.py first."
        )
...
def run_ablated_pass(
    ...
    """
    Forward pass with specified heads replaced by their mean activations.
    ...
    with torch.no_grad():
        with model.trace([input_ids], remote=remote):
            for layer_idx in layers_to_patch:
                o_proj_inp = get_o_proj_inputs(model, layer_idx)[0][0]
                # [1, seq_len, n_heads * head_dim]
                bsz, seq_len = o_proj_inp.shape[0], o_proj_inp.shape[1]
                ...
                set_o_proj_inputs(model, layer_idx, ((patched,), {}))

            logits = model.output.logits[0, -1, :].save()

    # In local execution .save() returns a concrete Tensor; in remote mode it
    # returns a SaveProxy that needs .value — handle both.
...
def main(args):
    ...
    if args.remote:
        ...
        model._ndif_remote = False
        remote_ctx = None

    tok = build_tok(model)

    head_means = load_mean_activations(model_name)
    ...
    for row in summary_rows:
        print(
            f"{row['label']:<35} {row['copy_success_rate']:>8.1%} "
            f"{row['hallucination_rate']:>8.1%} {row['n_ablated']:>8}"
        )
